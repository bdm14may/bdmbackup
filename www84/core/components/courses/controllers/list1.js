 ...
 */
.controller('mmCoursesListCtrl1', function($scope, $mmCourses, $mmCoursesDelegate, $mmUtil, $mmEvents, $mmSite, $q,
            mmCoursesEventMyCoursesUpdated, mmCoreEventSiteUpdated,$mmprogram,$stateParams) {
    ...
    $mmprogram.getcourcesbyprogramid($stateParams.courseid).then(function(res){
...
}).factory('$mmprogram', function($q, $mmSite, $log, $mmSitesManager, mmCoursesSearchPerPage, mmCoursesEnrolInvalidKey,$http,$mmSite) {
    var siteinfo1 = $mmSite.getInfo();
    
return{
    getcourcesbyprogramid:getcourcesbyprogramid
}

function getcourcesbyprogramid(id){
    return $http.get(siteinfo1.siteurl+"/webservice/rest/server.php?wstoken="+localStorage.getItem('token')+"&wsfunction=core_view_program_courses&programid="+id+"&moodlewsrestformat=json").then(function (res){
        return res;
